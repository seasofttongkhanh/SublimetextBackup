...
class CommandMyPlugin(sublime_plugin.WindowCommand):
    ...
    def run(self, *args, **kwargs):
        try:
            ...
            self.add_command = kwargs.get('add_command', None)
    ...
    def on_command(self, command):
        ...
        if self.fields_accept is True:
            return self.window.show_input_panel(self.fields_label, "", self.on_fields, None, None)
    ...
    def on_fields(self, fields):
        if fields != '':
            if self.add_command != '':
                self.args.append(self.add_command)
